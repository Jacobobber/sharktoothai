...
export const storeRepairOrderDetails = async (
  ...
  await client.query(
    `UPDATE app.repair_orders
     SET ro_open_date = $2,
         ...
         discount_total = $16,
         total_due = $17
     WHERE ro_id = $1 AND tenant_id = $18`,
    [
      roId,
      toDateOnly(payload.openTimestamp) ?? null,
      toDateOnly(payload.closeTimestamp) ?? null,
    ...
    await client.query(
      `INSERT INTO app.ro_parts_lines
       (part_line_id, tenant_id, ro_id, labor_index, part_index, part_line_number, part_number,
        quantity, unit_price, line_total, part_source, backorder_flag)
  ...
  await client.query(
    `INSERT INTO app.ro_deterministic_v2
       (ro_id, tenant_id, ro_number, ro_status, open_timestamp, close_timestamp, writeup_timestamp,
